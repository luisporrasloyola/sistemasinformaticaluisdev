-- Permisos de trabajo de Central Térmica Santa Rosa.
-- Incluye historial independiente de vigencias y múltiples archivos por cierre.
CREATE TABLE IF NOT EXISTS empresa_maquirenta_santa_rosa_permisos_trabajo (
    ...
    PRIMARY KEY (id),
    KEY idx_emsr_permisos_vigencia (fecha_inicio, fecha_vencimiento),
    KEY idx_emsr_permisos_registro (fecha_registro),
    KEY idx_emsr_permisos_usuario (registered_by_user_id),
    CONSTRAINT fk_emsr_permisos_usuario FOREIGN KEY (registered_by_user_id)
...
CREATE TABLE IF NOT EXISTS empresa_maquirenta_santa_rosa_permiso_vigencias (
    ...
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    UNIQUE KEY uq_emsr_permiso_vigencia_fin (permiso_trabajo_id, fecha_vencimiento),
    KEY idx_emsr_permiso_vigencia_actual (permiso_trabajo_id, fecha_vencimiento, id),
    KEY idx_emsr_permiso_vigencia_usuario (registered_by_user_id),
    CONSTRAINT fk_emsr_permiso_vigencia_permiso FOREIGN KEY (permiso_trabajo_id)
        REFERENCES empresa_maquirenta_santa_rosa_permisos_trabajo (id) ON DELETE CASCADE,
    CONSTRAINT fk_emsr_permiso_vigencia_usuario FOREIGN KEY (registered_by_user_id)
        REFERENCES users (id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS empresa_maquirenta_santa_rosa_permiso_archivos (
    id INT UNSIGNED NOT NULL AUTO_INCREMENT,
    ...
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    KEY idx_emsr_permiso_archivo_vigencia (vigencia_id, created_at),
    KEY idx_emsr_permiso_archivo_usuario (uploaded_by_user_id),
    CONSTRAINT fk_emsr_permiso_archivo_vigencia FOREIGN KEY (vigencia_id)
        REFERENCES empresa_maquirenta_santa_rosa_permiso_vigencias (id) ON DELETE CASCADE,
    CONSTRAINT fk_emsr_permiso_archivo_usuario FOREIGN KEY (uploaded_by_user_id)
        REFERENCES users (id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Migra registros creados con la primera versión sin eliminar fechas ni adjuntos.
INSERT INTO empresa_maquirenta_santa_rosa_permiso_vigencias
...
WHERE NOT EXISTS (
    SELECT 1 FROM empresa_maquirenta_santa_rosa_permiso_vigencias v
    WHERE v.permiso_trabajo_id = p.id
);

INSERT INTO empresa_maquirenta_santa_rosa_permiso_archivos
...
FROM empresa_maquirenta_santa_rosa_permisos_trabajo p
JOIN empresa_maquirenta_santa_rosa_permiso_vigencias v
...
WHERE COALESCE(p.archivo_path, '') <> ''
  AND NOT EXISTS (
      SELECT 1 FROM empresa_maquirenta_santa_rosa_permiso_archivos a
