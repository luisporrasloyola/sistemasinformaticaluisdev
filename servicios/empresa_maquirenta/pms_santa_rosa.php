...
require_module_access('empresa_maquirenta.pms_santa_rosa');
...
if ($action === 'list') {
    $q = db()->query("SELECT i.*, COALESCE(u.name,'') AS registered_by FROM empresa_maquirenta_csantarosa_pms i LEFT JOIN users u ON u.id=i.registered_by_user_id ORDER BY i.nro_pms DESC");
    json_response(['ok' => true, 'rows' => $q->fetchAll()]);
}

if ($action === 'get') {
    $q = db()->prepare('SELECT * FROM empresa_maquirenta_csantarosa_pms WHERE id=:id');
...
if ($action === 'save') {
    ...
    try {
        if (isset($_FILES['adjunto']) && $_FILES['adjunto']['error'] !== UPLOAD_ERR_NO_FILE) {
            $file = upload_file($_FILES['adjunto'], 'empresa_maquirenta_pms_santa_rosa', $allowedMimes);
            $newPath = $file['path'];
            $fileName = $file['name'];
        }
        
        if ($id > 0) {
            $q = db()->prepare('SELECT archivo_path FROM empresa_maquirenta_csantarosa_pms WHERE id=:id');
            $q->execute(['id' => $id]);
            $old = $q->fetch();
            if (!$old) {
                if ($newPath) {
            ...
            $sql = 'UPDATE empresa_maquirenta_csantarosa_pms SET rango_inicio=:start, rango_fin=:end, nro_pms=:pms, observaciones=:obs';
            $params = [
                'start' => $start,
                'end' => $end,
                'pms' => $pms,
            ...
            db()->prepare('INSERT INTO empresa_maquirenta_csantarosa_pms (rango_inicio, rango_fin, nro_pms, observaciones, archivo_path, archivo_nombre_original, registered_by_user_id) VALUES (:start, :end, :pms, :obs, :path, :name, :user)')
              ->execute([
                  'start' => $start,
                  'end' => $end,
                  'pms' => $pms,
...
if ($action === 'delete' || $action === 'delete_file') {
    ...
    if ($action === 'delete') {
        db()->prepare('DELETE FROM empresa_maquirenta_csantarosa_pms WHERE id=:id')->execute(['id' => $id]);
    } else {
        db()->prepare('UPDATE empresa_maquirenta_csantarosa_pms SET archivo_path=NULL, archivo_nombre_original=NULL WHERE id=:id')->execute(['id' => $id]);
    }
...
if ($action === 'download') {
    ...
    $sql = "SELECT i.id, i.nro_pms, i.rango_inicio, i.rango_fin, i.archivo_path, i.archivo_nombre_original FROM empresa_maquirenta_csantarosa_pms i WHERE i.archivo_path IS NOT NULL AND i.archivo_path<>''";
    $params = [];
    if ($ids) {
        $holders = [];
    ...
    header('Content-Disposition: attachment; filename="PMS_Santa_Rosa.zip"');
...
if ($action === 'file') {
    $q = db()->prepare('SELECT archivo_path, archivo_nombre_original FROM empresa_maquirenta_csantarosa_pms WHERE id=:id');
    $q->execute(['id' => (int) ($_GET['id'] ?? 0)]);
    $row = $q->fetch();
    $root = realpath(UPLOAD_PATH);
