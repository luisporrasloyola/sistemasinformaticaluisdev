...
if ($action === 'get') {
    $q=db()->prepare('SELECT * FROM empresa_maquirenta_charla_preoperacional WHERE id=:id');$q->execute(['id'=>(int)($_GET['id']??0)]);$row=$q->fetch();if(!$row)json_response(['ok'=>false,'message'=>'No se encontró el informe.'],404);json_response(['ok'=>true,'row'=>$row]);
}
if ($action === 'save') {
...
if ($action==='delete'||$action==='delete_file') {$id=(int)($_POST['id']??0);$q=db()->prepare('SELECT archivo_path FROM empresa_maquirenta_charla_preoperacional WHERE id=:id');$q->execute(['id'=>$id]);$row=$q->fetch();if(!$row)json_response(['ok'=>false,'message'=>'No se encontró el informe.'],404);if($action==='delete')db()->prepare('DELETE FROM empresa_maquirenta_charla_preoperacional WHERE id=:id')->execute(['id'=>$id]);else db()->prepare('UPDATE empresa_maquirenta_charla_preoperacional SET archivo_path=NULL,archivo_nombre_original=NULL WHERE id=:id')->execute(['id'=>$id]);delete_uploaded_file($row['archivo_path']??null);json_response(['ok'=>true]);}
if ($action === 'download') {
    ...
    $sql="SELECT i.id,i.nro_pms,i.rango_inicio,i.rango_fin,i.archivo_path,i.archivo_nombre_original,m.nombre AS maquinaria FROM empresa_maquirenta_charla_preoperacional i JOIN empresa_maquirenta_charla_preoperacional_maquinarias m ON m.id=i.maquinaria_tipo_id WHERE i.maquinaria_tipo_id=:machine AND i.archivo_path IS NOT NULL AND i.archivo_path<>''";
