...
$stmt = db()->prepare('SELECT name FROM empresa_maquirenta_formato_empresas WHERE id = :id');
$stmt->execute(['id' => $id]);
$company = $stmt->fetch();
if (!$company) {
...
$stmt = db()->prepare('SELECT COUNT(*) FROM empresa_maquirenta_formato_personal WHERE company_id = :id');
$stmt->execute(['id' => $id]);
...
db()->prepare('UPDATE empresa_maquirenta_formato_empresas SET status = 0 WHERE id = :id')->execute(['id' => $id]);
