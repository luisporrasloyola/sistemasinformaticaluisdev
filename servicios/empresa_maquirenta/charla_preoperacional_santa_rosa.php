...
if ($action === 'list') {
    $q=db()->prepare("SELECT i.*,m.nombre AS equipo,COALESCE(u.name,'') AS registered_by FROM empresa_maquirenta_santa_rosa_charla_preoperacional i JOIN empresa_maquirenta_santa_rosa_charla_preoperacional_maquinarias m ON m.id=i.maquinaria_tipo_id LEFT JOIN users u ON u.id=i.registered_by_user_id WHERE i.maquinaria_tipo_id=:id ORDER BY i.nro_pms DESC");$q->execute(['id'=>(int)($_GET['maquinaria_tipo_id']??0)]);json_response(['ok'=>true,'rows'=>$q->fetchAll()]);
}
if ($action === 'get') {
    $q=db()->prepare('SELECT * FROM empresa_maquirenta_santa_rosa_charla_preoperacional WHERE id=:id');$q->execute(['id'=>(int)($_GET['id']??0)]);$row=$q->fetch();if(!$row)json_response(['ok'=>false,'message'=>'No se encontró el informe.'],404);json_response(['ok'=>true,'row'=>$row]);
}
if ($action === 'save') {
...
if ($action === 'download') {
    ...
    $sql="SELECT i.id,i.nro_pms,i.rango_inicio,i.rango_fin,i.archivo_path,i.archivo_nombre_original,m.nombre AS maquinaria FROM empresa_maquirenta_santa_rosa_charla_preoperacional i JOIN empresa_maquirenta_santa_rosa_charla_preoperacional_maquinarias m ON m.id=i.maquinaria_tipo_id WHERE i.maquinaria_tipo_id=:machine AND i.archivo_path IS NOT NULL AND i.archivo_path<>''";
