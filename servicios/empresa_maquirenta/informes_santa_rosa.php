...
require_any_module_access(['empresa_maquirenta.informes_santa_rosa', 'empresa_maquirenta.seguridad']);
...
if ($action === 'list_machinery') {
    json_response(['ok'=>true, 'rows'=>db()->query("SELECT id,nombre FROM empresa_maquirenta_santa_rosa_informes_maquinarias WHERE estado=1 ORDER BY id")->fetchAll()]);
}
if ($action === 'add_machinery') {
    ...
    $q=db()->prepare('SELECT id,estado FROM empresa_maquirenta_santa_rosa_informes_maquinarias WHERE LOWER(nombre)=LOWER(:nombre) LIMIT 1');$q->execute(['nombre'=>$name]);$old=$q->fetch();
    if ($old) { if ((int)$old['estado']===1) json_response(['ok'=>false,'message'=>'Esta maquinaria ya existe.'],409); db()->prepare('UPDATE empresa_maquirenta_santa_rosa_informes_maquinarias SET nombre=:nombre,estado=1 WHERE id=:id')->execute(['nombre'=>$name,'id'=>$old['id']]); json_response(['ok'=>true,'id'=>(int)$old['id']]); }
    db()->prepare('INSERT INTO empresa_maquirenta_santa_rosa_informes_maquinarias(nombre) VALUES(:nombre)')->execute(['nombre'=>$name]);json_response(['ok'=>true,'id'=>(int)db()->lastInsertId()]);
}
if ($action === 'list') {
    $q=db()->prepare("SELECT i.*,m.nombre AS equipo,COALESCE(u.name,'') AS registered_by FROM empresa_maquirenta_santa_rosa_informes i JOIN empresa_maquirenta_santa_rosa_informes_maquinarias m ON m.id=i.maquinaria_tipo_id LEFT JOIN users u ON u.id=i.registered_by_user_id WHERE i.maquinaria_tipo_id=:id ORDER BY i.nro_pms DESC");$q->execute(['id'=>(int)($_GET['maquinaria_tipo_id']??0)]);json_response(['ok'=>true,'rows'=>$q->fetchAll()]);
}
if ($action === 'get') {
    $q=db()->prepare('SELECT * FROM empresa_maquirenta_santa_rosa_informes WHERE id=:id');$q->execute(['id'=>(int)($_GET['id']??0)]);$row=$q->fetch();if(!$row)json_response(['ok'=>false,'message'=>'No se encontró el informe.'],404);json_response(['ok'=>true,'row'=>$row]);
}
if ($action === 'save') {
    ...
    $newPath=null;
    try {
        $file=upload_file($_FILES['adjunto']??[],'empresa_maquirenta_santa_rosa_informes',document_attachment_mimes());$newPath=$file['path'];
        if($id){$q=db()->prepare('SELECT archivo_path FROM empresa_maquirenta_santa_rosa_informes WHERE id=:id AND maquinaria_tipo_id=:machine');$q->execute(['id'=>$id,'machine'=>$machine]);$old=$q->fetch();if(!$old){if($newPath)delete_uploaded_file($newPath);json_response(['ok'=>false,'message'=>'No se encontró el informe.'],404);} $sql='UPDATE empresa_maquirenta_santa_rosa_informes SET rango_inicio=:start,rango_fin=:end,nro_pms=:pms,observaciones=:obs';$params=['start'=>$start,'end'=>$end,'pms'=>$pms,'obs'=>trim((string)($_POST['observaciones']??'')),'id'=>$id];if($newPath){$sql.=',archivo_path=:path,archivo_nombre_original=:name';$params['path']=$newPath;$params['name']=$file['name'];}$sql.=' WHERE id=:id';db()->prepare($sql)->execute($params);if($newPath)delete_uploaded_file($old['archivo_path']??null);
        } else {db()->prepare('INSERT INTO empresa_maquirenta_santa_rosa_informes(maquinaria_tipo_id,rango_inicio,rango_fin,nro_pms,observaciones,archivo_path,archivo_nombre_original,registered_by_user_id) VALUES(:machine,:start,:end,:pms,:obs,:path,:name,:user)')->execute(['machine'=>$machine,'start'=>$start,'end'=>$end,'pms'=>$pms,'obs'=>trim((string)($_POST['observaciones']??'')),'path'=>$newPath,'name'=>$file['name'],'user'=>(int)(current_user()['id']??0)?:null]);}
...
if ($action==='delete'||$action==='delete_file') {$id=(int)($_POST['id']??0);$q=db()->prepare('SELECT archivo_path FROM empresa_maquirenta_santa_rosa_informes WHERE id=:id');$q->execute(['id'=>$id]);$row=$q->fetch();if(!$row)json_response(['ok'=>false,'message'=>'No se encontró el informe.'],404);if($action==='delete')db()->prepare('DELETE FROM empresa_maquirenta_santa_rosa_informes WHERE id=:id')->execute(['id'=>$id]);else db()->prepare('UPDATE empresa_maquirenta_santa_rosa_informes SET archivo_path=NULL,archivo_nombre_original=NULL WHERE id=:id')->execute(['id'=>$id]);delete_uploaded_file($row['archivo_path']??null);json_response(['ok'=>true]);}
if ($action === 'download') {
    ...
    $sql="SELECT i.id,i.nro_pms,i.rango_inicio,i.rango_fin,i.archivo_path,i.archivo_nombre_original,m.nombre AS maquinaria FROM empresa_maquirenta_santa_rosa_informes i JOIN empresa_maquirenta_santa_rosa_informes_maquinarias m ON m.id=i.maquinaria_tipo_id WHERE i.maquinaria_tipo_id=:machine AND i.archivo_path IS NOT NULL AND i.archivo_path<>''";
...
if ($action==='file') {$q=db()->prepare('SELECT archivo_path,archivo_nombre_original FROM empresa_maquirenta_santa_rosa_informes WHERE id=:id');$q->execute(['id'=>(int)($_GET['id']??0)]);$row=$q->fetch();$root=realpath(UPLOAD_PATH);$path=$row?realpath(dirname(__DIR__,2).DIRECTORY_SEPARATOR.$row['archivo_path']):false;if(!$path||!$root||!str_starts_with($path,$root)||!is_file($path)){http_response_code(404);exit('Archivo no encontrado.');}$mime=mime_content_type($path)?:'application/octet-stream';header('Content-Type: '.$mime);header('Content-Disposition: '.((int)($_GET['download']??0)===1?'attachment':'inline').'; filename="'.basename((string)($row['archivo_nombre_original']?:$path)).'"');readfile($path);exit;}
