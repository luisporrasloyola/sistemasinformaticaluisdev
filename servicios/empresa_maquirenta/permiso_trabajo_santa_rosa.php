...
if ($action === 'list') {
    ...
    $countStmt = db()->prepare('SELECT COUNT(*) FROM empresa_maquirenta_santa_rosa_permisos_trabajo p' . $where);
    ...
    $sql = "SELECT p.id,p.permiso_nombre,p.fecha_registro,p.registered_by_user_id,
        ...
        $statusSql AS estado,
        (SELECT COUNT(*) FROM empresa_maquirenta_santa_rosa_permiso_vigencias vh WHERE vh.permiso_trabajo_id=p.id) AS vigencia_count,
        (SELECT COUNT(*) FROM empresa_maquirenta_santa_rosa_permiso_archivos ta JOIN empresa_maquirenta_santa_rosa_permiso_vigencias tv ON tv.id=ta.vigencia_id WHERE tv.permiso_trabajo_id=p.id) AS total_documentos
      FROM empresa_maquirenta_santa_rosa_permisos_trabajo p
      JOIN empresa_maquirenta_santa_rosa_permiso_vigencias v ON v.id=(
          SELECT v2.id FROM empresa_maquirenta_santa_rosa_permiso_vigencias v2
...
if ($action === 'history') {
    ...
    $stmt=db()->prepare("SELECT v.id,v.fecha_inicio,v.fecha_vencimiento,v.fecha_cierre,v.observaciones,COALESCE(v.fecha_registro,DATE(v.created_at)) AS fecha_registro_etapa,v.created_at,p.fecha_registro AS permiso_fecha_registro,
        ...
        (SELECT COUNT(*) FROM empresa_maquirenta_santa_rosa_permiso_archivos ad WHERE ad.vigencia_id=v.id) AS documento_count,
        $statusSql AS estado
      FROM empresa_maquirenta_santa_rosa_permiso_vigencias v
    ...
    $fileStmt=db()->prepare("SELECT a.id,a.vigencia_id,a.archivo_nombre_original,a.tipo_archivo,a.created_at,COALESCE(u.name,'') uploaded_by FROM empresa_maquirenta_santa_rosa_permiso_archivos a LEFT JOIN users u ON u.id=a.uploaded_by_user_id JOIN empresa_maquirenta_santa_rosa_permiso_vigencias v ON v.id=a.vigencia_id WHERE v.permiso_trabajo_id=:id ORDER BY a.created_at,a.id");
...
if ($action === 'upload') {
    ...
    try{
        ...
        if($wantsClose)$pdo->prepare('UPDATE empresa_maquirenta_santa_rosa_permiso_vigencias SET fecha_cierre=:date,closed_by_user_id=:user WHERE id=:id')->execute(['date'=>$closeDate,'user'=>(int)(current_user()['id']??0)?:null,'id'=>(int)$current['id']]);
...
if ($action === 'delete') {
    $permitId=(int)($_POST['id']??0);
    $stmt=db()->prepare('SELECT a.archivo_path FROM empresa_maquirenta_santa_rosa_permiso_archivos a JOIN empresa_maquirenta_santa_rosa_permiso_vigencias v ON v.id=a.vigencia_id WHERE v.permiso_trabajo_id=:id UNION SELECT archivo_path FROM empresa_maquirenta_santa_rosa_permisos_trabajo WHERE id=:id2 AND archivo_path IS NOT NULL');
    $stmt->execute(['id'=>$permitId,'id2'=>$permitId]);$paths=$stmt->fetchAll(PDO::FETCH_COLUMN);
    $deleted=db()->prepare('DELETE FROM empresa_maquirenta_santa_rosa_permisos_trabajo WHERE id=:id');$deleted->execute(['id'=>$permitId]);
...
if ($action === 'file') {
    $stmt=db()->prepare('SELECT archivo_path,archivo_nombre_original FROM empresa_maquirenta_santa_rosa_permiso_archivos WHERE id=:id');$stmt->execute(['id'=>(int)($_GET['id']??0)]);$row=$stmt->fetch();
...
function permit_latest_vigencia(int $permitId,bool $withCounts=false):array|false{
    $sql="SELECT v.*,(SELECT COUNT(*) FROM empresa_maquirenta_santa_rosa_permiso_archivos a WHERE a.vigencia_id=v.id) archivo_count,(SELECT COUNT(*) FROM empresa_maquirenta_santa_rosa_permiso_archivos ad WHERE ad.vigencia_id=v.id) documento_count,(SELECT COUNT(*) FROM empresa_maquirenta_santa_rosa_permiso_vigencias vx WHERE vx.permiso_trabajo_id=v.permiso_trabajo_id) vigencia_count FROM empresa_maquirenta_santa_rosa_permiso_vigencias v WHERE v.permiso_trabajo_id=:id ORDER BY v.fecha_vencimiento DESC,v.id DESC LIMIT 1";
...
function permit_upload_files(array $input,int $limit,array $mimes,bool $required=false):array{
    ...
    $uploaded=[];try{foreach($indexes as $i)$uploaded[]=upload_file(['name'=>$input['name'][$i],'type'=>$input['type'][$i]??'','tmp_name'=>$input['tmp_name'][$i]??'','error'=>$input['error'][$i]??UPLOAD_ERR_NO_FILE,'size'=>$input['size'][$i]??0],'empresa_maquirenta_santa_rosa_permisos_trabajo',$mimes);return$uploaded;}catch(Throwable $e){permit_delete_new_files($uploaded);throw$e;}
}
function permit_store_files(int $vigenciaId,array $files,string $type='vigencia'):void{
    if(!$files)return;$stmt=db()->prepare('INSERT INTO empresa_maquirenta_santa_rosa_permiso_archivos(vigencia_id,archivo_path,archivo_nombre_original,tipo_archivo,uploaded_by_user_id) VALUES(:vigencia,:path,:name,:type,:user)');foreach($files as $file)$stmt->execute(['vigencia'=>$vigenciaId,'path'=>$file['path'],'name'=>$file['name'],'type'=>$type,'user'=>(int)(current_user()['id']??0)?:null]);
}
