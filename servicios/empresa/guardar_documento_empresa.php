...
if (!current_user_can_document('empresa.documentos', $documentoId, 'upload')) {
...
try {
    $pdf = upload_file($_FILES['pdf'] ?? [], 'empresa_documentos', document_attachment_mimes());

    if ($id > 0) {
        $currentStmt = db()->prepare('SELECT archivo_path FROM empresa_documentos WHERE id = :id');
        ...
        $sql = 'UPDATE empresa_documentos SET documento_id = :documento_id, fecha_registro = :fecha_registro, fecha_inicio = :fecha_inicio, fecha_fin = :fecha_fin, observaciones = :observaciones';
        $params = [
        ...
        $stmt = db()->prepare('INSERT INTO empresa_documentos (empresa_id, documento_id, fecha_registro, fecha_inicio, fecha_fin, observaciones, archivo_path, archivo_nombre_original, registered_by_user_id)
