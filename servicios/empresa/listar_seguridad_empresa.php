...
function empresa_seguridad_status(string $endDate, int $documentId): array
{
    return status_alert_document_status($endDate, 'empresa.seguridad', $documentId);
}
...
$rows = $stmt->fetchAll();
foreach ($rows as &$row) {
    $row['status'] = empresa_seguridad_status($row['fecha_fin'], (int) $row['documento_id']);
}
