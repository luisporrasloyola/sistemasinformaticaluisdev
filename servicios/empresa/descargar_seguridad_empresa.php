...
$sql = "SELECT ed.id, ed.archivo_path, ed.archivo_nombre_original, edc.nombre AS documento, e.razon_social, e.ruc
    ...
    WHERE ed.empresa_id = :empresa_id AND ed.archivo_path IS NOT NULL AND ed.archivo_path <> ''";
...
foreach ($rows as $row) {
    ...
    $files[] = [
        'path' => $fullPath,
        'name' => security_download_name($row['documento']) . '.' . $extension,
...
$zipName = security_download_name($files[0]['ruc'] . '_' . $files[0]['razon_social']) . '_seguridad.zip';
$zipContent = build_security_zip($files);
...
function security_download_name(string $value): string
{
    $value = trim($value);
...
function build_security_zip(array $files): string
{
    $data = '';
    $centralDirectory = '';
    $offset = 0;
    ...
    foreach ($files as $file) {
        $name = unique_security_zip_name($file['name'], $usedNames);
        $content = file_get_contents($file['path']);
        if ($content === false) continue;
        $crc = crc32($content);
        $size = strlen($content);
        [$dosTime, $dosDate] = security_dos_datetime((int) filemtime($file['path']));
...
function unique_security_zip_name(string $name, array &$usedNames): string
{
    $candidate = $name;
    $index = 2;
...
function security_dos_datetime(int $timestamp): array
{
