...
$assignment = $report['assignment']; $summary = $report['summary']; $rows = $report['individual_rows']; $note = $report['note']; $trips = $report['trips'] ?? [];
...
if ($trips) {
    ...
    foreach ($trips as $trip) {
        ...
        $tripRows .= '<tr><td>' . date('d/m/Y', strtotime($trip['trip_date'])) . '</td><td>' . $h($trip['schedule_label']) . '</td><td>' . date('H:i', strtotime($trip['started_at'])) . '</td><td>' . ($trip['ended_at'] ? date('H:i', strtotime($trip['ended_at'])) : '-') . '</td><td>' . $h($trip['duration_label']) . '</td><td>' . $h($trip['location_name']) . '</td><td>' . $h($trip['first_destination']) . '</td><td>' . $h($project) . '</td><td>' . $h($status) . '</td></tr>';
    }
    $tripSection = '<h2 class="section">Desplazamientos laborales</h2><table class="detail"><thead><tr><th>Fecha</th><th>Horario</th><th>Inicio</th><th>Fin</th><th>Duración</th><th>Origen</th><th>Destino</th><th>Proyecto</th><th>Estado</th></tr></thead><tbody>' . $tripRows . '</tbody></table>';
}
...
<table class="indicators"><tr><td style="border-left:1px solid #d7dfeb"><span>Puntualidad</span><strong>' . $h($summary['punctuality']) . '%</strong></td><td style="border-left:1px solid #d7dfeb"><span>Jornadas finalizadas</span><strong>' . $h($summary['compliance']) . '%</strong></td><td style="border-left:1px solid #d7dfeb"><span>Minutos de tardanza</span><strong>' . (int)$summary['late_minutes'] . ' min</strong></td><td style="border-left:1px solid #d7dfeb"><span>Horas extras</span><strong>' . $h(attendance_report_minutes_label((int)$summary['overtime_minutes'])) . '</strong></td></tr></table>
<h2 class="section">Detalle diario</h2><table class="detail"><thead><tr><th>Fecha</th><th>Día</th><th>Horario</th><th>Tolerancia</th><th>Lugar de marcación</th><th>Entrada</th><th>Salida</th><th>Tardanza</th><th>Horas extras</th><th>Asistencia</th><th>Jornada</th><th style="width:15%">Observación</th></tr></thead><tbody>' . $bodyRows . '</tbody></table>
' . $tripSection . '
