...
if ($trips) {
    ...
    foreach ($trips as $trip) {
        $visited = array_map(static fn(array $stop): string => (string)$stop['destination'] . (!empty($stop['activity']) ? ' - ' . (string)$stop['activity'] : ''), $trip['stops'] ?? []);
        $tripRows .= '<tr><td>' . date('d/m/Y', strtotime($trip['trip_date'])) . '</td><td>' . date('H:i', strtotime($trip['started_at'])) . '</td><td>' . ($trip['ended_at'] ? date('H:i', strtotime($trip['ended_at'])) : '-') . '</td><td>' . $h($trip['location_name']) . '</td><td>' . $h($trip['first_destination']) . '</td><td>' . $h($trip['reason']) . '</td><td>' . $h($visited ? implode(' | ', $visited) : '-') . '</td><td>' . ($trip['status']==='finalizado' ? 'Finalizado' : 'En curso') . '</td></tr>';
    }
    $tripSection = '<h2 class="section">Desplazamientos laborales</h2><table class="detail"><thead><tr><th>Fecha</th><th>Inicio</th><th>Fin</th><th>Origen</th><th>Primer destino</th><th>Motivo</th><th>Puntos visitados</th><th>Estado</th></tr></thead><tbody>' . $tripRows . '</tbody></table>';
}
