...
if ($trips) {
    ...
    foreach ($trips as $trip) {
        ...
        $tripRows .= '<tr><td>' . date('d/m/Y', strtotime($trip['trip_date'])) . '</td><td>' . $h($trip['schedule_label']) . '</td><td>' . date('H:i', strtotime($trip['started_at'])) . '</td><td>' . ($trip['ended_at'] ? date('H:i', strtotime($trip['ended_at'])) : '-') . '</td><td>' . $h($trip['duration_label']) . '</td><td>' . $h($trip['location_name']) . '</td><td>' . $h($trip['first_destination']) . '</td><td>' . $h($reason) . '</td><td>' . $h($status) . '</td></tr>';
    }
    $tripSection = '<h2 class="section">Salidas temporales</h2><table class="detail"><thead><tr><th>Fecha</th><th>Horario</th><th>Inicio</th><th>Fin</th><th>Duración</th><th>Origen</th><th>Destino</th><th>Motivo</th><th>Estado</th></tr></thead><tbody>' . $tripRows . '</tbody></table>';
}
