...
<form class="dashboard-filters attendance-report-filters" method="get">
    <div class="attendance-report-grid">
        ...
        <div><label class="form-label">Trabajador</label><select class="form-select select2-searchable" name="trabajador_id" data-placeholder="Buscar trabajador" data-no-results="No se encontraron trabajadores"><option value="0">Todos</option><?php foreach ($catalogs['workers'] as $item): ?><option value="<?= (int) $item['id'] ?>" <?= $workerId === (int) $item['id'] ? 'selected' : '' ?>><?= e($item['full_name'] . ' - ' . $item['document_number']) ?></option><?php endforeach; ?></select></div>
        <div><label class="form-label">Empresa</label><select class="form-select select2-searchable" name="empresa_id" data-placeholder="Buscar empresa" data-no-results="No se encontraron empresas"><option value="0">Todas</option><?php foreach ($catalogs['companies'] as $item): ?><option value="<?= (int) $item['id'] ?>" <?= $companyId === (int) $item['id'] ? 'selected' : '' ?>><?= e($item['name']) ?></option><?php endforeach; ?></select></div>
        <div><label class="form-label">Lugar de marcación</label><select class="form-select select2-searchable" name="punto_id" data-placeholder="Buscar lugar de marcación" data-no-results="No se encontraron lugares"><option value="0">Todos</option><?php foreach ($catalogs['locations'] as $item): ?><option value="<?= (int) $item['id'] ?>" <?= $locationId === (int) $item['id'] ? 'selected' : '' ?>><?= e($item['name']) ?></option><?php endforeach; ?></select></div>
