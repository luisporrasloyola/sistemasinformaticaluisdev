...
if ($id > 0) {
    $stmt = db()->prepare('SELECT * FROM empresa_maquirenta_formato_personal WHERE id = :id');
    $stmt->execute(['id' => $id]);
    $worker = $stmt->fetch() ?: $worker;
    $stmt = db()->prepare('SELECT position_id FROM empresa_maquirenta_formato_personal_puestos WHERE worker_id = :id');
    $stmt->execute(['id' => $id]);
...
$companies = db()->query('SELECT * FROM empresa_maquirenta_formato_empresas WHERE status = 1 ORDER BY name')->fetchAll();
$positions = db()->query('SELECT * FROM empresa_maquirenta_formato_puestos WHERE status = 1 ORDER BY name')->fetchAll();
...
<div class="page-title">
    ...
    <a class="btn btn-outline-secondary" href="<?= APP_URL ?>/modulos/empresa_maquirenta/personal.php"><i class="fa-solid fa-arrow-left me-2"></i>Volver</a>
</div>
...
<?php endif; ?>
<form class="work-panel needs-validation" id="personalForm" action="<?= APP_URL ?>/modulos/empresa_maquirenta/guardar_personal.php" method="post" enctype="multipart/form-data" novalidate>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-4">
            <h2>Foto y firma</h2>
            <?php foreach ([['photo','Foto','empresa_maquirenta/personal/fotos',$worker['photo_path']], ['signature','Firma digital','empresa_maquirenta/personal/firmas',$worker['signature_path']]] as $file): ?>
                <div class="file-box mb-3">
                    ...
                    <?php if ($file[3]): ?>
                        <div class="d-flex gap-2 mt-2">
                            <a class="btn btn-sm btn-outline-primary" href="<?= APP_URL . '/' . e($file[3]) ?>" download><i class="fa-solid fa-download"></i></a>
    ...
    <div class="d-flex justify-content-end gap-2 mt-4">
        <a class="btn btn-outline-secondary" href="<?= APP_URL ?>/modulos/empresa_maquirenta/personal.php">Cancelar</a>
...
(function () {
    ...
    function initCompanyQuickCreate() {
        ...
        deleteCompanyBtn?.addEventListener('click', async function () {
            ...
            const response = await fetch(`${window.personalServiceBase || baseUrl + "/servicios"}/eliminar_empresa.php`, { method: 'POST', body });
        ...
        companyQuickForm?.addEventListener('submit', async function (event) {
            ...
            try {
                ...
                const response = await fetch(`${window.personalServiceBase || baseUrl + "/servicios"}/guardar_empresa.php`, {
                    method: 'POST',
