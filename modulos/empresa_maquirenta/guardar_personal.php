...
require_once __DIR__ . '/../../servicios/empresa_maquirenta/formatos/file_safety.php';
require_role('Administrador');

if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    redirect('modulos/empresa_maquirenta/personal.php');
}
...
$duplicateStmt = db()->prepare('SELECT id, full_name FROM empresa_maquirenta_formato_personal WHERE document_number = :document_number AND id <> :id LIMIT 1');
...
if ($duplicateWorker) {
    ...
    $target = 'modulos/empresa_maquirenta/formulario_personal.php' . ($id > 0 ? '?id=' . $id . '&error=' : '?error=') . urlencode($message);
    redirect($target);
}
...
try {
    db()->beginTransaction();
    $companyId = resolve_catalog_id('empresa_maquirenta_formato_empresas', $companyInput);
    $photo = upload_file($_FILES['photo'] ?? [], 'empresa_maquirenta/personal/fotos', ['image/jpeg','image/png','image/webp']);
    $signature = upload_file($_FILES['signature'] ?? [], 'empresa_maquirenta/personal/firmas', ['image/jpeg','image/png','image/webp']);
    ...
    if ($id > 0) {
        $currentStmt = db()->prepare('SELECT photo_path, signature_path FROM empresa_maquirenta_formato_personal WHERE id = :id');
        ...
        $sql = 'UPDATE empresa_maquirenta_formato_personal SET company_id=:company_id, full_name=:full_name, document_type=:document_type,
        ...
        if ($signature['path']) {
            delete_empresa_maquirenta_formato_file($current['signature_path'] ?? null);
        ...
        $stmt = db()->prepare('INSERT INTO empresa_maquirenta_formato_personal
    ...
    db()->prepare('DELETE FROM empresa_maquirenta_formato_personal_puestos WHERE worker_id = :id')->execute(['id' => $id]);
    $insertPosition = db()->prepare('INSERT INTO empresa_maquirenta_formato_personal_puestos (worker_id, position_id) VALUES (:worker_id, :position_id)');
    foreach ($positionInputs as $positionInput) {
        $positionId = resolve_catalog_id('empresa_maquirenta_formato_puestos', (string) $positionInput);
    ...
    $target = 'modulos/empresa_maquirenta/formulario_personal.php' . ($id > 0 ? '?id=' . $id . '&error=' : '?error=') . urlencode($e->getMessage());
    redirect($target);
    ...
    if ($e->getCode() === '23000') {
        redirect('modulos/empresa_maquirenta/formulario_personal.php' . ($id > 0 ? '?id=' . $id . '&error=' : '?error=') . urlencode('El Nro. Documento ingresado ya está registrado. Verifique el personal existente o edite el registro correspondiente.'));
    }
