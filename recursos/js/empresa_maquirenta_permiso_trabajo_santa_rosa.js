document.addEventListener('DOMContentLoaded', function () {
    ...
    function render() {
        if (!rows.length) {
            body.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-5">No hay permisos de trabajo registrados.</td></tr>';
            return;
        }
        body.innerHTML = rows.map(row => {
            ...
            return '<tr class="permiso-main-row"><td>' + title + '<small class="d-block text-muted ms-1">' + validityCount + ' ' + (validityCount === 1 ? 'vigencia' : 'vigencias') + ' · ' + totalDocuments + ' documento(s)</small></td>' +
                '<td>' + displayDate(row.fecha_registro) + '</td><td>' + displayDate(row.fecha_inicio) + '</td><td>' + displayDate(row.fecha_vencimiento) + '</td><td>' + displayDate(row.fecha_cierre) + '</td>' +
                '<td>' + stateBadge(row.estado) + '</td><td>' + escapeHtml(row.registered_by || '—') + '</td><td><div class="permiso-actions">' + actions + '</div></td></tr>' +
                '<tr class="permiso-detail-row d-none" id="permisoDetail' + id + '"><td colspan="8"><div class="permiso-detail-content"></div></td></tr>';
        }).join('');
    ...
    async function load() {
        ...
        body.innerHTML = '<tr><td colspan="8" class="text-center py-5"><span class="spinner-border spinner-border-sm text-primary me-2"></span>Cargando permisos...</td></tr>';
        ...
        try {
            ...
            body.innerHTML = '<tr><td colspan="8" class="text-center text-danger py-5">' + escapeHtml(error.message) + '</td></tr>';
    ...
    async function toggleDetails(button) {
        ...
        try {
            ...
            content.innerHTML = '<div class="vigencias-inline">' +
                '<div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Etapa</th><th>Fecha de registro</th><th>Fecha de inicio</th><th>Fecha de vencimiento</th><th>Fecha de cierre</th><th>Estado</th><th>Documentos cargados</th></tr></thead><tbody>' +
                validities.map((validity, index) => {
                    ...
                    const links = regular.map(file => '<a class="btn btn-sm btn-outline-primary me-1 mb-1" href="' + cfg.serviceUrl + '?action=file&id=' + file.id + '" title="' + escapeHtml(file.archivo_nombre_original) + '"><i class="fa-solid fa-file-arrow-down"></i></a>').join('');
                    const closeLinks = closure.map(file => '<a class="btn btn-sm btn-outline-success me-1 mb-1" href="' + cfg.serviceUrl + '?action=file&id=' + file.id + '" title="Documento de cierre: ' + escapeHtml(file.archivo_nombre_original) + '"><i class="fa-solid fa-lock me-1"></i>Cierre</a>').join('');
                    const registrationDate = index === 0 ? validity.permiso_fecha_registro : String(validity.created_at || '').slice(0,10);
                    const rowClass = index === 0 ? 'vigencia-inicial-row' : 'vigencia-ampliacion-row vigencia-ampliacion-' + (((index - 1) % 3) + 1);
                    return '<tr class="' + rowClass + '"><td><strong>' + (index === 0 ? 'Vigencia inicial' : 'Ampliación ' + index) + '</strong></td><td>' + displayDate(registrationDate) + '</td><td>' + displayDate(validity.fecha_inicio) + '</td><td>' + displayDate(validity.fecha_vencimiento) + '</td><td>' + displayDate(validity.fecha_cierre) + '</td><td>' + stateBadge(validity.estado) + '</td><td><span class="document-count-pill">' + files.length + '</span> ' + links + closeLinks + '</td></tr>';
    ...
    forms.upload.onsubmit = async event => {
        ...
        try {
            ...
            const decision = await Swal.fire({
                ...
                inputAttributes: {min:row.fecha_inicio,max:localToday()},
                showCancelButton:true, showDenyButton:!forced,
                confirmButtonText:'<i class="fa-solid fa-lock me-1"></i> Cargar y cerrar',
                denyButtonText:'<i class="fa-solid fa-folder-open me-1"></i> Mantener abierta',
                ...
                preConfirm:value=>{if(!value){Swal.showValidationMessage('Seleccione la fecha de cierre.');return false}if(value<row.fecha_inicio||value>localToday()){Swal.showValidationMessage('La fecha debe estar entre el inicio de la vigencia y hoy.');return false}return value}
            });
